...
###########################################################
calc_mubp <- function(STATE, POP, COVARS) #POP=ANES/CPS/NORC, COVARS=demog/demog+party
{
  if (STATE=="AK" & POP=="ANES") stop("ANES data not available for AK")
  ...
  # with weighted (population) sufficient statistics computed from specified source for a given state.
  ...
  {
    # NOTE: processed CCES data set comes from SAS program dataprep_cces.sas.
    popdata <- read_csv("./data/cces_all.csv") %>% filter(state==STATE)
  }
  ...
  } else if (COVARS=="demog+party")
  {
    ...
    means.pop <- c(svymean(~male, pop.des, na.rm=T)[1],     #ref=female
                   ...
                   svymean(~hisp, pop.des, na.rm=T)[1],
                   svymean(~other, pop.des, na.rm=T)[1],
                   svymean(~demparty_nolean, pop.des, na.rm=T)[1], #ref=repparty_nolean
                   svymean(~otherparty_nolean, pop.des, na.rm=T)[1])
    # variance-covariance matrix
    varcov.pop <- svyvar(~ male + age1 + age2 + age3 + age4 + age5 + somecoll + coll + postcoll + black + hisp + other + demparty_nolean + otherparty_nolean, na.rm=TRUE, design=pop.des)
  }
  ...
  # read in Roper data for a given state (low response rate probability sample subject to selection bias).
  # NOTE: See dataprep_roper.sas for processing.
  poll <- read_csv("./data/roper_all.csv") %>% rename(state=STATE) %>% filter(state==STATE)
  if (COVARS=="demog")
  {
    poll <- poll %>% dplyr::select(-c("demparty_nolean","repparty_nolean","otherparty_nolean"))
  }
  ...
  } else if (COVARS=="demog+party")
  {
    ...
    fit <- glm(trumpind ~ male + age1 + age2 + age3 + age4 + age5 + somecoll + coll + postcoll + black + hisp + other + demparty_nolean + otherparty_nolean, family=binomial(link="probit"), data=poll_nomiss)
  }
  # save probit fit for checking
  fit_tidy <- tidy(fit) %>% mutate(state=STATE, pop=POP, covars=COVARS)
  write_csv(fit_tidy, file=paste("results_check_US/Roper_fit_",STATE,"_",POP,"_",COVARS,".csv",sep=""))
  ...
  write_csv(draws, file=paste("results_check_US/Roper_draws_",STATE,"_",POP,"_",COVARS,".csv",sep=""))
...
set.seed(217531)
results <- tibble()
# AK -- no ANES data, NORC and CPS only
results <- bind_rows(results,calc_mubp("AK", "NORC", "demog"))
results <- bind_rows(results,calc_mubp("AK", "NORC", "demog+party"))
results <- bind_rows(results,calc_mubp("AK", "CPS", "demog"))
# AZ
results <- bind_rows(results,calc_mubp("AZ", "ANES", "demog"))
results <- bind_rows(results,calc_mubp("AZ", "ANES", "demog+party"))
results <- bind_rows(results,calc_mubp("AZ", "NORC", "demog"))
results <- bind_rows(results,calc_mubp("AZ", "NORC", "demog+party"))
results <- bind_rows(results,calc_mubp("AZ", "CPS", "demog"))
# CCES - AZ only
set.seed(531)
results <- tibble()
results <- bind_rows(results,calc_mubp("AZ", "CCES", "demog"))
results <- bind_rows(results,calc_mubp("AZ", "CCES", "demog+party"))


## coverage indicators
roper_all_states <- results %>% 
...
roper_all_states %>% dplyr::select(starts_with("cover"))
## save
write_csv(roper_all_states, file="./results_roper.csv")
