...
##############################
# Example using Real NSFG Data
##############################

# read input data (NSFG)

data_path = "https://github.com/bradytwest/IndicesOfNISB/raw/master/fem_comp.csv";
nsfg <- read.csv(data_path, h=T)

# all user inputs (need dummies)
selection_col <- "smartphone"
depvar_col <- "WRK12MOS"
z_cols <- c("educat_3","marcat_4")
a_cols <- c("kidflag")
# number of Z coefficients (for predictors of interest)
zparams <- 2

# data frames for selected and non-selected cases
sel <- nsfg[which(nsfg[,selection_col]==1),c(depvar_col,z_cols,a_cols)]
ns <- nsfg[which(nsfg[,selection_col]==0),c(z_cols,a_cols)]

mean_YZA_s <- c(mean(sel[,depvar_col]), colMeans(sel[,z_cols]), mean(sel[,a_cols]))
var_YZA_s <- var(sel)

# NOTE: These means, variances, and covariances could also be input manually for non-selected cases
mean_ZA_ns <- c(colMeans(ns[,z_cols]), mean(ns[,a_cols]))
var_ZA_ns <- var(ns)

stats_selected <- list(n_s = dim(sel)[1], mean_YZA_s = mean_YZA_s, var_YZA_s=var_YZA_s)
stats_not_selected <- list(n_ns = dim(ns)[1], mean_ZA_ns = mean_ZA_ns, var_ZA_ns=var_ZA_ns)

finaldraws <- mub_reg_bayes(stats_selected, stats_not_selected, zparams, userphi=NA, ndraws=100)

# Posterior median and credible interval for MUB for intercept
cat("Intercept:","\n")
quantile(finaldraws[,1], c(0.5,0.025,0.975))

# Posterior medians and credible intervals for MUB for the two (in this example) Z coefficients
cat(z_cols[1],"coefficient:","\n")
quantile(finaldraws[,2], c(0.5,0.025,0.975))
cat(z_cols[2],"coefficient:","\n")
quantile(finaldraws[,3], c(0.5,0.025,0.975))